...
class ModelANN(nn.Module):
    def __init__(self, X):
        ...
        # self.linear = nn.Sequential(
        #     nn.Linear(self.target_size, 5),
        #     nn.LeakyReLU(),
        #     nn.Linear(5, 1)
        # )
        # self.linear = nn.Sequential(
        #     nn.Linear(self.target_size, 4),
        #     nn.LeakyReLU(),
        #     nn.Linear(4, 4),
        #     nn.LeakyReLU(),
        #     nn.Linear(4, 1)
        # )
        self.epoch = my_utils.get_epoch(rows, self.target_size)
        self.lr = my_utils.get_lr(rows, self.target_size)
