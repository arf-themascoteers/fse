...
class BSNetFC2(nn.Module):
    ...
    def forward(self, X):
        channel_weights = self.bam(X)
        channel_weights, output = self.recnet(channel_weights, X)
        return channel_weights, output

    def bam(self, X):
        return self.weighter(X)

    def recnet(self, channel_weights, X):
