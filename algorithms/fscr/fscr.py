...
class FSCR:
    def __init__(self, rows, original_feature_size, target_feature_size, sigmoid=True):
        ...
        self.model = ANN(rows, self.original_feature_size, self.target_feature_size, sigmoid)
    ...
    def fit(self, X, y, X_validation, y_validation):
        ...
        for epoch in range(self.epochs):
            y_hat = self.model(X,spline, row_size)
    ...
    def evaluate(self,X, spline,y,size):
        self.model.eval()
        y_hat = self.model(X, spline, size)
    ...
    def write_columns(self):
        ...
        for index,p in enumerate(self.model.machines):
    ...
    def dump_row(self, epoch, spline, y, spline_test, y_test, row_size, row_test_size):
        train_r2, train_rmse = self.evaluate(self.X,spline, y, row_size)
        test_r2, test_rmse = self.evaluate(self.X_validation, spline_test, y_test, row_test_size)
